...
public class SimpleCustomChromeTabsHelper {
    ...
    public CustomTabsIntent buildCustomTabIntent(CustomTabsSession customTabSession) {
    ...
    public void openUrl(String url) {
        if (sPackageNameToUse == null && getPackageNameToUse(mContext) == null){
            signalFallback(mContext, Uri.parse(url));
            return;
        }
    ...
    private void signalFallback(Activity context, Uri uri) {
        if(mFallback != null){
            mFallback.openUri(context, uri);
        }
    }
